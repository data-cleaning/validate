...
- investigate, summarise, visualise rules and data validation results.
- import, export, rule sets from and to various formats;
...
Validate rulesets in `validator` objects. The `validator()` function
...
w <- v
```
then `w` is not a copy of `v`, just another name for the same physical object
as `v`.  To make an actual copy, you can select everything.
```
w <- v[]
```
...
Or, merge with the summarized results. The result of `summary` is a `data.frame`. 
```{r}
