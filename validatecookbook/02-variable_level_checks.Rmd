...
Variable checks are checks that can be performed on a variable by variable
basis. An example is checking that a variable called `Age` is nonnegative,
or of integer type. Variable checks are among the simplest checks.
...
More complicated cases of completeness checks are discussed in Chapter \@ref(sect:completeness).


## Field length

The length of the content of character fields can be tested using either R's
standard `nchar()` function, or with the convenience function
`field_length`.
...
summary(out)
```
One advantage of `chech_field_length` is that its argument is converted to
...
**Techincal note.** The field length is measured as the number of [code
...
In this specification the `"d"` stands for a _d_igit, any other character
except the asterisks stands for itself, and the asterisks stands for zero or
...
To check for a maximum number of decimals, one can use regular expression syntax
with `grepl`. For example to specify a number with one digit, then a decimal point,
and then maximal two digits, use the following rule.
```{r}
rule <- validator(
  grepl("^\\d\\.\\d{,2}$", x)
)
...
This function is convenient only for very simple number formats. For generic
pattern matching in strings, see the next section.
...
the asterisks wildcard character (`*`) is interpreted as 'one or more characters'
and the question mark (`?`) is interpreted as 'a character'.
...
is equivalent to `incl.prob >= 0` and `incl.prob <= 0`. This can be toggled
...
**Technical note.** For numeric ranges it is often a better idea to work with
inclusive inequalities ($\leq$, $\geq$) then with strict inequalities ($<$,
$>$).  Take as an example the strict inequality demand `income > 0`. This means
that _any_ income larger than zero is acceptable, including numbers such as
$0.01$, $0.000001$ and $10^{-\textrm{Googol}}$. In practice there is almost
always a natural minimal acceptable value that is usually dictated by the unit
of measurement. For example, if we measure income in whole Euros, a better
demand would be `income >= 1`.
