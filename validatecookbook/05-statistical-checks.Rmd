...
## Aggregates and hierarchical code lists
...
provinces. Such relations can be defined as parent-child relationships. For
Samplonia we can do this as follows.

```{r}
samplonia <- data.frame(
  code   = c("Agria", "Induston"
              , "Wheaton", "Greenham"
...
samplonia
```
The function `hierarchy` can be used to check whether values for
children aggregate to values for their parents.
