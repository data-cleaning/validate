...
```
An error has occurred while executing the rule.
```{r}
summary(out)
```
Apparently, in one or more of the groups defined by `period` and `measure`
the parent `` occurs multiple times (see \@ref(sect:uniqueness)). Let us
