...
SDMX registries. To obtain a code list, users must specify the following.
...
In the following rule we retrieven the codelist of economic activiteis
...
Equivalently, you could use `global_codelist()` to avoid specifying the API endpoint. The output
...
```{r, eval=FALSE}
rules <- validator_from_dsd(endpoint = endpoint("ESTAT")
