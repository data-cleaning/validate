...
Use R's standard `is.na()` to check missin items in individual variables. Negate
...
characters' and the question mark (`?`) is interpreted as 'a character'.

For example, to check that the `id` variable in `SBS2000` starts with `RET` we
can do the following.

```{r}
rule <- validator( field_format(id, "RET*"),field_format(size, "sc?" ) )
out  <- confront(SBS2000, rule)
summary(out)
```
...
```{r}
rule <- validator(  grepl("^sc[0-9]$", size)
...
```{r}
rules <- validator(TO = turnover >= 0, TC=total.costs >= 0)
```
...
```{r}
rules <- rules + validator(PR = in_range(incl.prob, min=0, max=1))
```
By default, `in_range` includes the boundaries of the range, so the above rule
is equivalent to `incl.prob >= 0` and `incl.prob <= 0`. 
...
inequalities ($\leq$, $\geq$) then with strict inequalities ($<$, $>$).  Take
...
formats.
```{r}
in_range(period, min="2017Q2", max = "2018Q2", format="auto")
```
The formats recognized with `format="auto"` are described in the help file, see `?in_range`.
...
# pass the codelist
out <- confront(SBS2000, rule, ref=list(valid_codes=codelist))
summary(out)
```
...
```{r}
rule <- validator(
  contains_at_least( keys = data.frame(period = as.character(2014:2019))
...
where all years are present, and `FALSE` for each block where one of the
...
```{r}
rule <- validator(contains_at_least(keys=minimal_keys, by=measure))
out <- confront(samplonomy, rule, ref=list(minimal_keys=keyset))
# showing 7 columns of output for readability
summary(out)[1:7]
```
...
out  <- confront(samplonomy, rule, ref=list(all_keys=keyset))
# showing 7 columns of output for readability
summary(out)[1:7]
```
...
```{r}
is_linear_sequence(c("2020Q4","2020Q2","2020Q3","2020Q1"), begin="2020Q2")
```
...
is not a linear series. This is where the function `in_linear_series` comes in.
```{r}
...
```{r}
rule <- validator(in_linear_sequence(period, by=list(region, freq, measure)))
out  <- confront(samplonomy, rule)
summary(out)[1:7]
```
...
summary(out)[1:7]
```
Observer that we use `glob(forbidden_keys)` to tell `does_not_contain` that the
...
like `validator` objects are lists of validation rules, `indicicator` objects
...
A specific problem that may occur is when the result of an indicator is
non-numeric.
```{r}
jj <- indicator(mh = mean(height), a = return("A"))
```
here, the second 'indicator' is an expression that yields a constant character
string `"A"`.
```{r}
...
objects. To demonstrate this, we will create a two more versions of the
`SBS2000` dataset.
```{r}
version4 <- version3
