...
There are a few subtleties to keep in mind when interpreting uniqueness. The
...
In R, the choice is made to treat `NA` as an actual value when checking for duplicates
or uniqe records (see `?duplicated` from base R). To see this inspect the following
code and output.
```{r}
df1 <- data.frame(x=rep(1,3), y = c("A", NA, NA))
is_unique(df1$x, df1$y)
```
